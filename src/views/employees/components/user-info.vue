<template>
  <div class="user-info">
    ...
    <el-row type="flex" justify="end">
      <el-button type="primary" size="small" icon="el-icon-printer" @click="toPrintPage">打印</el-button>
    </el-row>
    <el-divider />
    <!-- 个人信息 -->
  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      if (this.formData?.staffPhoto) {
        // 有的话就将值赋给fileList
        this.$refs.detailInfoRef.fileList = [{
    ...
    async savePersonal() {
      //  调用父组件
    // 保存的时候，判断有没有上传图片，如果有上传的话，就提示用户上传图片
      const fileList = this.$refs.detailInfoRef.fileList
      //   对item.upload取反，说明没有上传
      // const flag = fileList.some((item) => !item.upload)
      // if (flag) {
      if (!fileList.length) {
        return this.$message.warning('请上传图片')
      }
      await saveUserDetailById({ ...this.userInfo, staffPhoto: fileList[0]['url'] })
      this.$message.success('保存成功')
    },

    // 保存员工基本信息
    async saveUser() {
      await updatePersonal({ ...this.userInfo, id: this.userId })
      this.$message.success('保存成功')
    },
    toPrintPage() {
      this.$router.push(`/employees/print/${this.userId}?type=personal`)
    }
  }
}
</script>
